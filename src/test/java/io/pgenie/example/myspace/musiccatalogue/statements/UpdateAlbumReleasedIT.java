...
class UpdateAlbumReleasedIT extends AbstractDatabaseIT {
  ...
  void updateAlbumReleasedUpdatesRow() throws SQLException {
    var inserted = execute(new InsertAlbum("The Wall", null, null, null));
    ...
    var rows = execute(new UpdateAlbumRecordingReturning(null, inserted.id()));
  ...
  void updateAlbumReleasedNoMatchIsNoop() throws SQLException {
    long affected = execute(new UpdateAlbumReleased(LocalDate.of(2000, 1, 1), 99999L));
