 ...
 *
 * <p>{@link #normalModeCreatesServerSidePreparedStatements()} uses the regular datasource from the
 * base class to provide a contrast with the noPreparing datasource.
 */
class NoPreparingIT extends AbstractDatabaseIT {
      ...
      var result =
          execute(
              ...
              new InsertAlbum(
                  ...
                  AlbumFormat.VINYL,
                  new RecordingInfo(
                      "Britannia Row Studios", "London", "UK", LocalDate.of(1976, 7, 1))));
  ...
  void noPreparingSelectAlbumByFormatFindsInsertedAlbum() throws SQLException {
    try (HikariDataSource npDs = dsNoPreparing()) {
      var recording =
          new RecordingInfo("Rockfield Studio", "Monmouth", "UK", LocalDate.of(1975, 1, 1));
      var inserted =
          execute(
              npDs,
              new InsertAlbum(
                  "A Night at the Opera", LocalDate.of(1975, 11, 21), AlbumFormat.CD, recording));
  ...
  void noPreparingUpdateAlbumReleasedNoMatchIsNoop() throws SQLException {
    try (HikariDataSource npDs = dsNoPreparing()) {
      long affected = execute(npDs, new UpdateAlbumReleased(LocalDate.of(2000, 1, 1), 99999L));
      assertEquals(0L, affected);
    }
  }

  /**
   * Executes the same parameterised statement repeatedly within a single transaction (keeping the
   * same JDBC connection) and checks whether server-side prepared statements were created.
   *
   * <p>pgjdbc's default {@code prepareThreshold} is 5: after the 5th execution of the same SQL on
   * the same connection the driver sends a {@code Parse} message so the statement appears in {@code
   * pg_prepared_statements}. In {@code noPreparing} mode (simple query protocol) this must never
   * happen.
   */
  @Test
  void noPreparingDoesNotCreateServerSidePreparedStatements() throws SQLException {
    try (HikariDataSource npDs = dsNoPreparing();
      ...
      try (PreparedStatement ps =
              conn.prepareStatement("SELECT count(*) FROM pg_prepared_statements");
          ResultSet rs = ps.executeQuery()) {
        rs.next();
        count = rs.getLong(1);
      }
      assertEquals(0L, count, "noPreparing mode must not create server-side prepared statements");
    }
  }
  ...
  void normalModeCreatesServerSidePreparedStatements() throws SQLException {
    try (Connection conn = ds.getConnection()) {
      ...
      assertTrue(
          count > 0, "normal mode should create server-side prepared statements after threshold");
    }
  }
}
